...
LDTSCT_DATA <- LDTSCT_DATA[LDTSCT_DATA$corr_ans==1, ]
...
# Full model formula
form <- RT ~ taskDev * (cwDev + nwDev + pwDev) + task_order + stim_list + respgroup +
...
# Fit full model
m.full <- glmer(form, data = LDTSCT_DATA, family = Gamma(log), control = glmerControl(optimizer = c("bobyqa")))
...
m.cw <- glmer(form.cw, data = LDTSCT_DATA, family = Gamma(log), control = glmerControl(optimizer = c("bobyqa")))
m.cw.int <- update(m.cw, .~. -taskDev)
anova(m.cw, m.cw.int, test="Chi")

# Examine effect of task for pseudowords
m.pw <- glmer(form.pw, data = LDTSCT_DATA, family = Gamma(log), control = glmerControl(optimizer = c("bobyqa")))
m.pw.int <- update(m.pw, .~. -taskDev)
anova(m.pw, m.pw.int, test="Chi")

# Examine effect of task for nonwords
m.nw <- glmer(form.nw, data = LDTSCT_DATA, family = Gamma(log), control = glmerControl(optimizer = c("bobyqa")))
